...
- ## it is a useful measure for
  ...
  - Answer: ${n \choose 2}$ (number of ways of choosing a pair) happens when the array is inversely sorted
...
- ## Binary Multiplication: Break into smaller problems
  ...
  - So basically the observation is that a multiplication by $2^k$ would take a $O(k)$ time to just left shit the $k$ bits needed.
    - This is assuming that we are doing binary multiplication by storing the bits in array. 
    - Writing $k$ many $0/1$(s) directly to an array's pre-allocated slots requires $O(k)$-time
    - Example: $12 = 00001100. \quad 12 \times 2^3 = 96$ is the same as left shitfing $00001100$ by 3 bits
    - $00001100 \ll 3$
...
- ## Outline
  - quick review of [probability](../../../notes/probability.md)
  - the hiring problem
...
- ## quick review of probability theory:
  - **a discrete random variable** takes on countable number of distinct values (usually counts)
  - **Expectation:** The expectation $E[X]$ of a discrete random variable $X$ is defined as:
  >$$
  >E[X] = \sum{i \cdot Pr(X = i)}
  >$$
  - **Q (waiting time for first success):** coin comes up heads with probability $p$ and tails with probability $1 - p$.
    - how many flips $X$ until first head is seen (we call it **waiting time**, for later use)
    - **A:**
    >$$
    >\begin{aligned}
    >E[X] &= \sum_{j = 1}^{\infty}{j \cdot Pr[X = j]} \\
    >&= \sum_{j = 1}^{\infty}{j \cdot (1 - p)^{j - 1}p} \\
    >&= p\sum_{j = 1}^{\infty}{j \cdot (1 - p)^{j - 1}} \\
    >&= p \cdot \frac{1}{p^{2}} \\
    >&= \frac{1}{p}
    >\end{aligned}
    >$$
- ## [exercise, show $\sum_{j = 1}^{\infty}{j \cdot (1 - p)^{j - 1}} = \frac{1}{p^{2}}$](./problems/lecture_problems/lecture8_series.md)
- ## Linearity of expectation 
  - given two random variables $X$ and $Y$ (not necessarily independent)
  - $E[X + Y] = E[X] + E[Y]$
  - **remark:** $E[XY] = E[X]E[Y]$ only when $X$ and $Y$ are independent
- ## Indicator random variables
  - an indicator random variable $X$ only takes values $0$ or $1$ :
    - $E[X] = Pr[X = 1]$
- ## The hiring problem
  >````cpp
  >Hire-assistant(n):
  ...
  >      best <- i
  >````
  - Q: how many people are hired in the worst case and when:
    - A: $n$ people are hired when the quality of candidates are sorted in ascending order
  - Q: how to avoid the randomness of the input
    - A: insert randomness in the algorithm itself (? why would it work?)
  >````cpp
  >Hire-assistant(n):
  ...
  - A: similar to [guessing cards with memory](./problems/lecture_problems/lecture8_guessing_card_memory.md)
  - 